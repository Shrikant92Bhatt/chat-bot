...
export class UserManagementComponent {
  ...
  public isSelf(user: AdminUser): boolean {
    return this.auth.session()?.uid === user.uid;
  }
  ...
  public async changeRole(user: AdminUser, role: 'user' | 'admin'): Promise<void> {
    ...
    try {
      ...
      // every request), so send the operator to the access-denied screen
      // rather than letting the dashboard silently break around them.
