...
- Inline `[n]` citation markers (see below) are guaranteed self-consistent only when the model doesn't also populate its own ```ui` `sources` array for the same researched turn — nothing in the current prompts asks it to, so in practice this holds, but it's a real assumption, not an invariant the code enforces.
- Message feedback has a narrow theoretical race for a brand-new thread: `ThreadService.getThread` (the ownership check) only sees a thread once its first `PUT /threads` write lands, which happens on the first message send, not on thread creation — in practice that write completes well before an assistant reply (and its rateable action bar) exists, but it's a real window.

### Inline citations (`shared/markdown/citation-markers.ts`)
`research.ts`'s `formatFindings()` numbers every citation against the exact same deduped `collectSources()` array that becomes `data.sources` on the frontend (previously numbered per-query, with no relationship to that array — citing against the old numbering would have pointed at the wrong source). `research_findings:v2` tells the model it may cite `[n]` inline against that numbering. Client-side, `citation-markers.ts` DOM-walks already-sanitized Markdown HTML (never a regex over the HTML string) and turns an in-range `[n]` into a clickable chip that scrolls to the matching numbered Sources chip; anything out of range — including every marker on a message with no sources — renders as inert plain text. `<code>`/`<pre>`/`<a>` subtrees are always skipped.
